...
export default function PlasterFinishLabor({ project, setCostChange, costs }) {
    ...
    const getData = async (isSearch = false) => {
        ...
        try {
            const res = await axios.get(
                `/stafftwo/labor/plaster-finish/getdata?${params}`
            );
    ...
    const [plasterFinish, setPlasterFinishLabor] = useState(false);
    ...
    const showEditModal = (plasterFinish) => {
        setIsModalOpen(true);
        setPlasterFinishLabor(plasterFinish);

        setQuantity(plasterFinish.quantity);
        setNoOfDays(plasterFinish.no_of_days);
        setRate(plasterFinish.rate);

        form.setFieldsValue({
            position: plasterFinish.position,
            quantity: plasterFinish.quantity,
            no_of_days: plasterFinish.no_of_days,
            rate: plasterFinish.rate,
        ...
        if (plasterFinish) {
            try {
                const res = await axios.put(
                    `/stafftwo/labor/plaster-finish/update/${plasterFinish.id}`,
                    values
                );
                if (res.data.status === "updated") {
                    ...
                    openNotification(
                        ...
                        "bottomRight",
                        "Updated!",
                        "The plaster and finish labor has been updated successfully."
                    );
                }
            } catch (err) {
                if (
            ...
            try {
                const res = await axios.post(
                    "/stafftwo/labor/plaster-finish/store",
                    values
                );
                if (res.data.status === "created") {
                    ...
                    openNotification(
                        ...
                        "bottomRight",
                        "Created!",
                        "The plaster finish and labor has been created successfully."
                    );
                }
            } catch (err) {
                if (
    ...
    const handleDelete = async (id) => {
        ...
        try {
            const res = await axios.delete(
                `/stafftwo/labor/plaster-finish/destroy/${id}`
            );
            ...
            if (res.data.status === "deleted") {
                ...
                openNotification(
                    ...
                    "bottomRight",
                    "Deleted!",
                    "The plaster and finish labor has been deleted successfully."
                );
            }
        } catch (err) {
            console.log(err);
    ...
    return (
        <>
            ...
            <div className="py-2 text-md font-bold uppercase">
                List of Plaster and Finish Labor
            </div>
            <div className="flex md:flex-row flex-col gap-2 mb-2">
                <Search
                    placeholder="Input plaster and finish labor position"
                    allowClear
                    enterButton="Search"
                    loading={searching}
            ...
            <div className="overflow-x-auto">
                <Table
                    ...
                    footer={() => (
                        <div>
                            <div>
                                Total Plaster and Finish Labor Budget:
                                {formatPeso(costs.PlasterFinishLaborBudget)}
                            </div>
                            <div>
                                Total Plaster and Finish Labor Cost:
                                ...
                                <Button
                                    ...
                                    onClick={() =>
                                        Modal.confirm({
                                            ...
                                            content:
                                                "Are you sure you want to delete this plaster and finish labor?",
                                            okText: "Yes",
                                            cancelText: "No",
                                            onOk() {
            ...
            <Modal
                title={
                    plasterFinish
                        ? "UPDATE CEMENT AND PLASTER FINISH LABOR"
                        : "CEMENT AND PLASTER FINISH LABOR LABOR"
                }
                width={800}
                open={isModalOpen}
